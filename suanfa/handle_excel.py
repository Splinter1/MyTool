...
class HandExcel:
    ...
    def load_excel(self, file):
        if file == "data":
            open_excel = openpyxl.load_workbook(data_file)
            return open_excel
        if file == "result":
            open_excel = openpyxl.load_workbook(result_file)
            return open_excel

    def get_sheet_data(self, file, index=None):
        sheet_name = self.load_excel(file).sheetnames
        if index is None:
            index = 0
        data = self.load_excel(file)[sheet_name[index]]
        return data
    ...
    def get_rows(self, file):
        rows = self.get_sheet_data(file).max_row
        return rows

    # 获取一行的内容
    def get_rows_value(self, file, row):
        row_list = []
        for i in self.get_sheet_data(file)[row]:
...
if __name__ == '__main__':
    ...
    # print(HandExcel().get_rows_value("data", 1))
    ...
    # print(HandExcel().get_cells_num("data", "是否执行"))
